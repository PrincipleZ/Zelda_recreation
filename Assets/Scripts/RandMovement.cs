...
public class RandMovement : MonoBehaviour
{
    ...
    void ChooseNextDestination()
    {
        while (!validDestination)
        {
			...
			if (path == Vector3.right && !Physics.Raycast(transform.position, transform.up, 0.6f, finalMask))
            {
                validDestination = true;
            }

			else if (path == Vector3.up && !Physics.Raycast(transform.position, transform.up, 0.6f, finalMask))
            {
                validDestination = true;
            }

			else if (path == Vector3.down && !Physics.Raycast(transform.position, -transform.up, 0.6f, finalMask))
            {
                validDestination = true;
            }

			else if (path == Vector3.left && !Physics.Raycast(transform.position, -transform.right, 0.6f, finalMask))
            {
                validDestination = true;
            }
